		...
		if(strstr(_b->first.c_str(), "anm_")==_b->first.c_str())
		{
			...
			for(u32 i=0; i<=8; ++i)
			{
				if(i==0)
					...
					sprintf_s				(buff,"%s%d",pm->m_base_name.c_str(),i);		
...
void player_hud::update_inertion(Fmatrix& trans) //--#SM+#--
{
	if (inertion_allowed())
	{
		...
		{ // Худ в режиме "Прицеливание"
			float factor = pMainHud->m_parent_hud_item->GetInertionFactor(); // до работать как sm
		...
		if (pMainHud != NULL)
		{
			float power_factor = pMainHud->m_parent_hud_item->GetInertionPowerFactor(); // пересмотреть

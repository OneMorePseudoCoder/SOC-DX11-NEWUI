...
bool CWeaponMagazined::TryReload() 
{
	if (m_pCurrentInventory)
	{
		...
		{
			m_pCurrentAmmo = smart_cast<CWeaponAmmo*>(m_pCurrentInventory->GetAny(m_ammoTypes[i].c_str()));
			if(m_pCurrentAmmo) 
			{ 
				m_ammoType			= i; 
				SetPending			(TRUE);
...
{
	if (smart_cast<CWeaponAmmo*>(m_pCurrentInventory->GetAny(m_ammoTypes[m_ammoType].c_str())))
		return	(true);
	else
		for(u32 i = 0; i < m_ammoTypes.size(); ++i)
			if (smart_cast<CWeaponAmmo*>(m_pCurrentInventory->GetAny(m_ammoTypes[i].c_str())))
				return	(true);
	return		(false);
}
...
void CWeaponMagazined::ReloadMagazine() 
{
	...
	if(!unlimited_ammo()) 
	{
		...
		m_pCurrentAmmo = smart_cast<CWeaponAmmo*>(m_pCurrentInventory->GetAny(tmp_sect_name));
...
bool CWeaponMagazined::GetBriefInfo(II_BriefInfo& info)
{
	...
	else
	{
		...
		{
			sprintf_s(ammo, "%d", GetAmmoCount(0));
			info.fmj_ammo._set(ammo);
		}
		if (at_size == 2)
		{
			sprintf_s(ammo, "%d", GetAmmoCount(1));
			info.ap_ammo._set(ammo);
		}
		if (at_size >= 3)
		{
			sprintf_s(ammo, "%d", GetAmmoCount(m_ammoType));
			info.fmj_ammo._set(ammo);
			u8 m = 0;
			u64 ap = 0;
			while (m < at_size)

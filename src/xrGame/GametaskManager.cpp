...
vGameTasks&	CGameTaskManager::GameTasks	() 
{
...
{
	FindTaskByID key(id);
	vGameTasks_it it = std::find_if(GameTasks().begin(),GameTasks().end(),key);
	if( it!=GameTasks().end() )
		return (*it).game_task;
...
{
	vGameTasks_it it			= GameTasks().begin();
	vGameTasks_it it_e		= GameTasks().end();
...
void CGameTaskManager::UpdateTasks						() 
{
	...
	vGameTasks_it		i = GameTasks().begin();
...
void CGameTaskManager::UpdateActiveTask				()
{
	...
	{
		bool bDone								=false;
		vGameTasks::iterator it			= GameTasks().begin();
		vGameTasks::iterator it_e		= GameTasks().end();

...
//.
		...
		tag_text                        = g_gameTaskXml->Read(l_root, "text", 0, NULL);
		if (tag_text)
			m_Description               = tag_text;
//.
...
//.
		if (i == 0)
		{
			m_icon_texture_name = g_gameTaskXml->Read(g_gameTaskXml->GetLocalRoot(), "icon", 0, NULL);
			if (m_icon_texture_name.size() &&
				0 != stricmp(*m_icon_texture_name, "ui\\ui_icons_task"))
			{
				icon_rect = CUITextureMaster::GetTextureRect(*m_icon_texture_name);
				icon_rect.rb.sub(icon_rect.rb, icon_rect.lt);
				m_icon_texture_name = CUITextureMaster::GetTextureFileName(*m_icon_texture_name);
			}
			else
				if (m_icon_texture_name.size()){
					icon_rect.x1 = g_gameTaskXml->ReadAttribFlt(l_root, "icon", 0, "x");
					icon_rect.y1 = g_gameTaskXml->ReadAttribFlt(l_root, "icon", 0, "y");
					icon_rect.x2 = g_gameTaskXml->ReadAttribFlt(l_root, "icon", 0, "width");
					icon_rect.y2 = g_gameTaskXml->ReadAttribFlt(l_root, "icon", 0, "height");
				}
		}
...
void SGameTaskKey::save(IWriter &stream)
{
	...
	save_data(game_task->m_icon_texture_name, stream);
	save_data(game_task->icon_rect, stream);
	save_data(game_task->m_Description, stream);
...
void SGameTaskKey::load(IReader &stream)
{
	...
	load_data(game_task->m_icon_texture_name, stream);
	load_data(game_task->icon_rect, stream);
	load_data(game_task->m_Description, stream);

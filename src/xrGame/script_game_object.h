...
//	_DECLARE_FUNCTION11	(SetSatiety,		void, float);
	...
	//передача порции информации InventoryOwner
			...
			void				GiveGameNews(LPCSTR caption, LPCSTR news, LPCSTR texture_name, Frect tex_rect, int delay, int show_time);
			void				GiveGameNews(LPCSTR caption, LPCSTR news, LPCSTR texture_name, Frect tex_rect, int delay, int show_time, int type);

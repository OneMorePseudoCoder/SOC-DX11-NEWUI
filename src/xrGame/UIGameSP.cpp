	...
	case kSCORES:
		{
			...
			SDrawStaticStruct* m_game_objective = AddCustomStatic("main_task", true);
			SGameTaskObjective* o = pActor->GameTaskManager().ActiveObjective();
			CGameTask* t1 = pActor->GameTaskManager().ActiveTask();
			m_game_objective->m_static->TextItemControl()->SetTextST((o) ? o->description.c_str() : "st_no_active_task");

			if (o && o->description.c_str())
			{
				...
				for (; it != Actor()->encyclopedia_registry->registry().objects_ptr()->end(); ++it)
				{
					...
					{
						SGameTaskObjective& o = t1->Objective(0);
						if (o.article_id.size() && it->article_id == o.article_id)
						{
							...
							sm2->m_static->TextItemControl()->SetTextST(A.data()->text.c_str());
						}
					}
				}
			}

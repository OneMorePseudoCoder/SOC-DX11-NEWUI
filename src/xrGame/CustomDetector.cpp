...
bool CCustomDetector::CheckCompatibilityInt(CHudItem* itm, u32* slot_to_activate)
{
	...
	if (!bres && slot_to_activate)
	{
		*slot_to_activate = NO_ACTIVE_SLOT;
		if (m_pCurrentInventory->ItemFromSlot(BOLT_SLOT))
			*slot_to_activate = BOLT_SLOT;  

		if (m_pCurrentInventory->ItemFromSlot(KNIFE_SLOT))
			*slot_to_activate = KNIFE_SLOT;

		if (m_pCurrentInventory->ItemFromSlot(RIFLE_SLOT) && m_pCurrentInventory->ItemFromSlot(RIFLE_SLOT)->GetSlot() != RIFLE_SLOT)
			*slot_to_activate = RIFLE_SLOT;

		if (m_pCurrentInventory->ItemFromSlot(PISTOL_SLOT) && m_pCurrentInventory->ItemFromSlot(PISTOL_SLOT)->GetSlot() != PISTOL_SLOT)

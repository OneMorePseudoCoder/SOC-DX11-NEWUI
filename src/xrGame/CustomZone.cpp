...
//////////////////////////////////////////////////////////////////////////
	...
	if (m_zone_flags.test(eIdleLight))
	{
		...
		m_zone_flags.set(eIdleLightVolumetric, READ_IF_EXISTS(pSettings, r_bool, section, "idle_light_volumetric", TRUE));
...
void CCustomZone::UpdateWorkload	(u32 dt)
{
	...
	if (Level().CurrentEntity()) {
		...
		if (m_effector && EnableEffector())
			m_effector->Update(m_fDistanceToCurEntity, radius, m_eHitTypeBlowout);
	}
...
#pragma todo(" <=====---- ТАК ДЛЯ БРОБЫ ИЗ ЗОВ ПРИПЯТИ   ----=====>")

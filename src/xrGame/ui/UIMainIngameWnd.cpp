...
void CUIMainIngameWnd::Update()
{
	...
	if (!(Device.dwFrame % 5))
	{
		...
		// ewiArtefact
		if ((GameID() == eGameIDArtefactHunt) && !(Device.dwFrame % 30)){
			bool b_Artefact = (NULL != pActor->inventory().ItemFromSlot(ARTEFACT_SLOT));
			if (b_Artefact)
...
void CUIMainIngameWnd::UpdateMainIndicators()
{
	...
	UpdateQuickSlots();
	//if (IsGameTypeSingle())
	//	HUD().GetUI()->UIGame()->PdaMenu().UpdateRankingWnd();

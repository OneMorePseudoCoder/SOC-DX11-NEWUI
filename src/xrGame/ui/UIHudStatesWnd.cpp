	...
	//	m_ui_weapon_third_ammo->Show(true); //Alundaio: third_ammo
		...
		if (wpnm)
		{
			if (wpnm->m_ammoType == 0){
				...
				m_ui_weapon_fmj_ammo->SetTextColor(color_rgba(238, 155, 23, 120));
			}
			if(wpnm->m_ammoType == 1){
				m_ui_weapon_ap_ammo->SetTextColor(color_rgba(238, 155, 23, 255));}
			else{
				m_ui_weapon_ap_ammo->SetTextColor(color_rgba(238, 155, 23, 120));
			}
		...
		if (wpn && wpn->m_bGrenadeMode)
		{
			...
			m_fire_mode->SetTextColor(color_rgba(238, 155, 23, 0));
		}
		else
		{
			m_ui_grenade->SetTextColor(color_rgba(238, 155, 23, 150));
			m_fire_mode->SetTextColor(color_rgba(238, 155, 23, 255));
		}		
	...
	if (pda)
	{
			...
			it != pda->feel_touch.end(); ++it)
		{
			...
			if (!monster || !monster->g_Alive())
				continue;
#pragma todo(" <=====---- monster->play_detector_sound() до делать  ----=====>")
...
void CUIHudStatesWnd::UpdateIndicatorType(CActor* actor, ALife::EInfluenceType type)
{
	...
	*/
	...
	if (hit_power <= protect)
	{
		...
		//		SwitchLA( false, type );
		sprintf_s(str, sizeof(str), "%s%s", texture, "green");
		texture = str;
	...
	if (hit_power - protect < m_zone_threshold[type])
	{
		...
		//		SwitchLA( false, type );
		sprintf_s(str, sizeof(str), "%s%s", texture, "yellow");
		texture = str;
		m_indik[type]->InitTexture(texture);
		actor->conditions().SetZoneDanger(0.0f, type);
		return;
	...
	//	SwitchLA( true, type );
	sprintf_s(str, sizeof(str), "%s%s", texture, "red");
	texture = str;
	m_indik[type]->InitTexture(texture);

...
struct SLuaWpnParams  //inventory_item
{
...
//	cur_wpn.get_upgrades_str(str_upgrades);
	...
	if (IsGameTypeSingle())
	{
		...
		string128 str;
		sprintf_s(str, sizeof(str), "%d", ammo_count);
		...
		sprintf_s(str, sizeof(str), "%s", pSettings->r_string(ammo_types[0].c_str(), "inv_name_short"));

...
{
	CUIXml			uiXml;
	uiXml.Load(CONFIG_PATH, UI_PATH, "pda_events_new.xml");
	CUIXmlInit		xml_init;
	...
	m_UINewsWnd = xr_new<CUIEncyclopediaWnd>(); m_UINewsWnd->SetAutoDelete(false);
	m_UINewsWnd->Init();
...
void CUIDiaryWnd::Draw()
{
	...
	pos = m_sign_places[eencyclopedia];
	pos.add(tab_pos);

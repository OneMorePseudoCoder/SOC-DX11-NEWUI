...
void CUIMapLocationHint::SetInfoMSpot(CMapSpot* spot)
{
	...
	CGameTask* gt = Actor()->GameTaskManager().HasGameTask(ml, true);
	if (gt)
		SetInfoTask(gt, ml->GetHint());
	else
		SetInfoStr(ml->GetHint());
}

void CUIMapLocationHint::SetInfoTask(CGameTask* task, LPCSTR text)
{
	SetInfoMode(2);
	CUIStatic* S = m_info["t_icon"];

	S->InitTexture(task->m_icon_texture_name.c_str());
	Frect r = task->icon_rect;
	Frect texture_rect;
	...
	SGameTaskObjective* o = Actor()->GameTaskManager().ActiveObjective();

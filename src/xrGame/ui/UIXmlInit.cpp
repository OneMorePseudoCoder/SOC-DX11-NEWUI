...
bool CUIXmlInit::InitProgressShape(CUIXml& xml_doc, LPCSTR path, int index, CUIProgressShape* pWnd){
	...
	pWnd->mockwise = xml_doc.ReadAttribInt(path, index, "clockwise") ? true : false;
...
void CUIXmlInit::InitAutoStaticGroup(CUIXml& xml_doc, LPCSTR path, int index, CUIWindow* pParentWnd)
{
	...
	const XML_NODE* node = curr_root->IterateChildren(NULL);
	int cnt_static = 0;
	int cnt_frameline = 0;
	int cnt_text = 0;
	string512							buff;
	...
	while (node)
	{
		...
		if (0 == stricmp(node_name, "auto_static"))
		{
			...
			sprintf(buff, "auto_static_%d", cnt_static);
			...
			if (0 == stricmp(node_name, "auto_frameline"))
			{
				...
				sprintf(buff, "auto_frameline_%d", cnt_frameline);

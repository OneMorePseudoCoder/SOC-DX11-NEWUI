...
public:
	...
	virtual void		Execute			(LPCSTR arguments)
	{
		...
		WriteRegistry_StrValue("InstallCDKEY", value);
...
public:
	...
	virtual void	Execute					(LPCSTR args) 
	{
		...
		//-----------------------------------------
		s32 GameTypeID = 0;
		if (!xr_strcmp(GameType, "deathmatch")) GameTypeID = eGameIDDeathmatch;
		else
			if (!xr_strcmp(GameType, "teamdeathmatch")) GameTypeID = eGameIDTeamDeathmatch;
			else
				if (!xr_strcmp(GameType, "artefacthunt")) GameTypeID = eGameIDArtefactHunt;
		//-----------------------------------------

		const SGameTypeMaps& M		= gMapListHelper.GetMapListFor((EGameIDs)GameTypeID);

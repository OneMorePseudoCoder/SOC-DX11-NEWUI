...
//.	m_dwDeltaTime = 0;
	...
	if(GO) 
	{
		...
		float power = (GameID() == eGameIDSingle) ? Power(GO->Position().distance_to(pos)) : 0.0f;
...
//.			m_dwDeltaTime = 0;
			...
			CreateHit(GO->ID(),ID(),dir,power, power_critical, BI_NONE,position_in_bone_space,impulse,ALife::eHitTypeRadiation);
		}
	}
}
...
void CRadioactiveZone::feel_touch_new					(CObject* O	)
{
	...
	if (GameID() != eGameIDSingle)
	{
		...
		{
			CreateHit(O->ID(),ID(),Fvector().set(0, 0, 0),0.0f,0.0f, BI_NONE,Fvector().set(0, 0, 0),0.0f,ALife::eHitTypeRadiation);
		}
	};
};

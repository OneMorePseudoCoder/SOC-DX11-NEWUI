...
//	} else m_spawn_probability			= 0.f;
	...
	m_melee_rotation_factor = (pSettings->line_exist(section, "Melee_Rotation_Factor")) ? int(pSettings->r_float(section, "Melee_Rotation_Factor")) : 1.5f;
	berserk_always = (pSettings->line_exist(section, "berserk_always")) ? int(pSettings->r_bool(section, "berserk_always")) : false;
}

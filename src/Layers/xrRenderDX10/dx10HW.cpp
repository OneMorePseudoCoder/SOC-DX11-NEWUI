...
#define ReleaseObject(aObject) if ((aObject) != nullptr) { aObject->Release(); aObject = nullptr; } 
...
void CHW::CreateDevice( HWND m_hWnd, bool move_window )
{
   ...
   HRESULT R;
    ...
    R =  D3D11CreateDeviceAndSwapChain(   nullptr,
		                                  m_DriverType,
		                                  nullptr,
                                          createDeviceFlags,
		                                  pFeatureLevels,
	...
	HRESULT hr = pDevice->QueryInterface(__uuidof(ID3D11Device1), reinterpret_cast<void**>(&pDevice1));
	if (SUCCEEDED(hr))
	{
		...
		pDevice1->QueryInterface(__uuidof(IDXGIDevice), (void **)&dxgiDevice);
		...
		selectResolution(swapChainDesc.Width, swapChainDesc.Height, bWindowed);
		...
		dxgiFactory->CreateSwapChainForHwnd(pDevice1, m_hWnd, &swapChainDesc, nullptr, nullptr, &DXGISwapChain);
